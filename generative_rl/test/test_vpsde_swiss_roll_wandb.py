        ...
        diffusion_model = dict(
            parameters = dict(
                path = dict(
                    parameters = dict(
                        type = dict(
                            values = ["linear_vp_sde","linear","gvp"],
                        ),
                    ),
                ),
                model = dict(
                    parameters = dict(
                        type = dict(
                            values = ["velocity_function", "score_function", "noise_function"],
                        ),
                    ),
...
def train_func():
    ...
    with wandb.init(
        ...
        def render_video(data_list, video_save_path, iteration, fps=100, dpi=100):
            ...
            wandb_run.log(
                data=dict(
                    video=wandb.Video(os.path.join(video_save_path, f"iteration_{iteration}.mp4"), fps=fps, format="mp4"),
                ),
                commit=False)
            # clean up
            plt.close(fig)
...
if __name__ == '__main__':
    sweep_id = wandb.sweep(sweep=sweep_config, project="swiss_roll_diffusion_model")
