################################################################################################
# This script demonstrates how to use an Independent Conditional Flow Matching (ICFM) with mask pretraining, which is a flow model, to train Swiss Roll dataset.
################################################################################################
...
config = EasyDict(
    dict(
        ...
        parameter=dict(
            ...
            checkpoint_path="./checkpoint-icfm-mask",
            video_save_path="./video-icfm-mask",
            device=device,
        ),
    )
)
...
if __name__ == "__main__":
    ...
    for iteration in track(range(config.parameter.iterations), description="Training"):
        ...
        # batch_data is of shape (batch_size, x_size)
        # create a mask for the batch data for random masking data elementwise
        mask = torch.rand_like(batch_data) > 0.5
        mask = mask.to(config.device)

        batch_data = batch_data.to(config.device)

        # use maskfill to fill the masked data with zeros
        condition_data = torch.masked_fill(batch_data, mask, 0.0)
        ...
        if iteration >= 0 and iteration % 1000 == 0:
            logp = compute_likelihood(
                ...
                condition=torch.zeros_like(torch.tensor(data).to(config.device)),
