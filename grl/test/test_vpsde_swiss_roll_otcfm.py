...
if __name__ == "__main__":
    ...
    flow_model = OptimalTransportConditionalFlowModel(config=config.flow_model).to(config.flow_model.device)
    ...
    for iteration in track(range(config.parameter.iterations), description="Training"):
        ...
        if iteration > 0 and iteration % config.parameter.eval_freq == 0:
            ...
            x_t = flow_model.sample_forward_process(t_span=t_span, batch_size=500).cpu().detach()
        ...
        optimizer.zero_grad()
        loss.backward()
        gradien_norm = torch.nn.utils.clip_grad_norm_(flow_model.parameters(), config.parameter.clip_grad_norm)
        optimizer.step()
