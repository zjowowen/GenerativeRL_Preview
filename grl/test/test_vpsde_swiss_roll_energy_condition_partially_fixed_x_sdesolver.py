...
def render_video(
    ...
    if info is not None:
        ani.save(
            os.path.join(
                video_save_path,
                f"iteration_{iteration}_guidance_scale_{guidance_scale}_{info}.mp4",
            ),
            fps=fps,
            dpi=dpi,
        )
    ...
    for train_iter in track(
        ...
        if (
            ...
            for guidance_scale in [0.0]:
                ...
                fixed_mask = (
                    torch.tensor([0.0, 1.0]).to(config.model.device).unsqueeze(0)
                )
                x_t = (
                    energy_conditioned_diffusion_model.sample_forward_process_with_fixed_x(
                        fixed_x=fixed_x,
        ...
        for train_iter in range(config.parameter.energy_guidance.iterations):
            ...
            if (
                ...
                for guidance_scale in config.parameter.evaluation.guidance_scale:
                    ...
                    subprocess_list.append(p)
                    fixed_x = (
                        torch.tensor([1.0, 0.0]).to(config.model.device).unsqueeze(0)
                    )
                    fixed_mask = (
                        torch.tensor([0.0, 1.0]).to(config.model.device).unsqueeze(0)
                    )
