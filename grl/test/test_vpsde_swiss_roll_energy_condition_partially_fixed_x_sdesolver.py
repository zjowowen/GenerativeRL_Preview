...
if __name__ == "__main__":
    ...
    for train_iter in track(range(config.parameter.unconditional_model.iterations), description="unconditional_model training"):
        ...
        unconditional_model_training_loss = energy_conditioned_diffusion_model.score_matching_loss(train_x)
    ...
    def generate_fake_x(model, sample_per_data):
        # model.eval()
        fake_x_sampled = []
        for i in track(range(config.parameter.support_size), description="Generate fake x"):
            #TODO: mkae it batchsize

            fake_x_sampled.append(model.sample(
                t_span = torch.linspace(0.0, 1.0, 32).to(config.model.diffusion_model.device),
                batch_size=sample_per_data,
                guidance_scale=0.0,
                with_grad=False,
                solver_config=EasyDict(dict(
                    type = "ODESolver",
                    args = dict(
                        library="torchdyn",
                    ),
    ...
    fake_x = generate_fake_x(
        ...
        config.parameter.sample_per_data)
    ...
    data_loader_fake_x = torch.utils.data.DataLoader(data_fake_x, batch_size=config.parameter.energy_guidance.batch_size, shuffle=True)
    data_generator = get_train_data(data_loader)
    ...
    with Progress() as progress:
        ...
        for train_iter in range(config.parameter.energy_guidance.iterations):
            ...
            train_data = next(data_generator).to(config.model.diffusion_model.device)
            train_x, train_value = train_data[:,:x_size], train_data[:,x_size]
            train_fake_x = next(data_generator_fake_x).to(config.model.diffusion_model.device)
            ...
            energy_guidance_loss = energy_conditioned_diffusion_model.energy_guidance_loss(x=train_fake_x,)
