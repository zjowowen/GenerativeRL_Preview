...
class DiT_Encoder(nn.Module):
    ...
    def forward(
        ...
        for i, block in enumerate(self.blocks):
            index = torch.tensor(i/len(self.blocks), device=x.device, dtype=torch.float32)
    ...
    with wandb.init(
        ...
        **config.wandb if hasattr(config, "wandb") else {}
        ...
        if config.parameter.train_mode == "ddp":
            ...
            vae = nn.parallel.DistributedDataParallel(vae.to(config.vae.device), device_ids=[torch.distributed.get_rank()])
        else:
