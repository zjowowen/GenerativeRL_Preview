...
if __name__ == "__main__":
    ...
    reference_model = DiffusionModel(config=config.diffusion_model).to(config.diffusion_model.device)
    reference_model = torch.compile(reference_model)
    # get data
    raw_data = make_swiss_roll(n_samples=config.parameter.data_num, noise=0.01)

    data = raw_data[0].astype(np.float32)[:,[0,2]]
    # transform data
    data[:,0] = data[:,0]/np.max(np.abs(data[:,0]))
    data[:,1] = data[:,1]/np.max(np.abs(data[:,1]))
    data = (data - data.min()) / (data.max() - data.min())
    data = data * 10 - 5
    t = np.expand_dims(raw_data[1], -1)
    data = np.concatenate((data, t), axis=-1)
    another_data = data.copy()
    np.random.shuffle(another_data)
    
    pairs_num = another_data.shape[0]
    
    train_data = np.zeros((pairs_num, 4))
    
    for i in range(pairs_num):
        # import pdb
        # pdb.set_trace()
        if abs(np.linalg.norm(data[i][:2], ord=2)- 3) <= abs(np.linalg.norm(another_data[i][:2], ord=2)- 3):
            
            train_data[i][:2] = data[i][:2]
            train_data[i][2:] = another_data[i][:2]
        else:
            train_data[i][2:] = data[i][:2]
            train_data[i][:2] = another_data[i][:2]
    ...
    if config.parameter.checkpoint_path is not None:
        ...
        last_iteration = -1
    data_loader = torch.utils.data.DataLoader(train_data, batch_size=config.parameter.batch_size, shuffle=True)
    ...
    for iteration in track(range(config.parameter.iterations), description="Training"):
        ...
        if iteration > 0 and iteration % config.parameter.eval_freq == 0 and iteration != 1000:
        ...
        elif config.parameter.training_loss_type=="dpo":
            loss = diffusion_model.dpo_loss(reference_model, batch_data, beta=2)
        else:
        ...
        log.info(f"iteration {iteration}, gradient {gradient_sum/counter}, loss {loss.item()}")
