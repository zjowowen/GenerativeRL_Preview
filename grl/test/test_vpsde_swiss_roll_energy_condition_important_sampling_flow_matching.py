...
if __name__ == "__main__":
    ...
    if config.parameter.evaluation.model_save_path is not None:
        ...
        if not os.path.exists(config.parameter.evaluation.model_save_path):
            ...
            if len(checkpoint_files) == 0:
                ...
                diffusion_model_important_sampling.load_state_dict(checkpoint["diffusion_model"])
    ...
    for train_iter in track(range(config.parameter.diffusion_model.iterations), description="diffusion model training"):
        ...
        diffusion_model_important_sampling_training_loss = diffusion_model_important_sampling.score_matching_loss(train_x, average=False)
        diffusion_model_important_sampling_training_loss = torch.mean(diffusion_model_important_sampling_training_loss * torch.exp(train_value))

        diffusion_model_important_sampling_optimizer.zero_grad()
        diffusion_model_important_sampling_training_loss.backward()
        gradien_norm = torch.nn.utils.clip_grad_norm_(diffusion_model_important_sampling.parameters(), config.parameter.diffusion_model.clip_grad_norm)
        diffusion_model_important_sampling_optimizer.step()
        moving_average_loss = 0.99 * moving_average_loss + 0.01 * diffusion_model_important_sampling_training_loss.item() if train_iter > 0 else diffusion_model_important_sampling_training_loss.item()

        if train_iter % 100 == 0:
            log.info(f"iteration {train_iter}, gradient norm {gradien_norm}, unconditional model loss {diffusion_model_training_loss.item()}, model loss {diffusion_model_important_sampling_training_loss.item()}, moving average loss {moving_average_loss}")
        ...
        if train_iter == 0 or (train_iter+1) % config.parameter.evaluation.eval_freq == 0:
            ...
            p = mp.Process(target=render_video, args=(x_t, config.parameter.evaluation.video_save_path, f"unconditional_diffusion_model_iteration_{diffusion_model_iteration}", 100, 100))
            p.start()
            subprocess_list.append(p)

            x_t = diffusion_model_important_sampling.sample_forward_process(t_span=t_span, batch_size=500).cpu().detach()
            x_t=[x.squeeze(0) for x in torch.split(x_t, split_size_or_sections=1, dim=0)]
            p1 = mp.Process(target=render_video, args=(x_t, config.parameter.evaluation.video_save_path, f"conditional_diffusion_model_iteration_{diffusion_model_iteration}", 100, 100))
            p1.start()
