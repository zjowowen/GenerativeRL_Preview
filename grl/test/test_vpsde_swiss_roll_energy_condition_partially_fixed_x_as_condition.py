...
config = EasyDict(
        ...
        value_function_model = dict(
            ...
            DoubleVNetwork = dict(
                ...
                backbone = dict(
                    type = "ConcatenateMLP",
                    args = dict(
                        hidden_sizes = [128*(x_size+constraint_size), 256, 256],
                        output_size = 1,
                        activation = "silu",
                    ),
                ),
        ...
        diffusion_model = dict(
            ...
            energy_guidance = dict(
                ...
                backbone = dict(
                    type = "ConcatenateMLP",
                    args = dict(
                        hidden_sizes = [x_size + constraint_size + t_embedding_dim, 256, 256],
                        output_size = 1,
                        activation = "silu",
                    ),
                ),
    ...
    parameter = dict(
        ...
        value_function_model = dict(
            batch_size = 256,
            stop_training_iterations = 10000,
            learning_rate = 5e-4,
        ...
        evaluation = dict(
            eval_freq=5000,
            video_save_path="./video-swiss-roll-energy-conditioned-fixed-x-as-condition-diffusion-model",
            model_save_path="./model-swiss-roll-energy-conditioned-fixed-x-as-condition-diffusion-model",
...
if __name__ == "__main__":
    ...
    energy_conditioned_diffusion_model = EnergyConditionalDiffusionModel(config.model.diffusion_model, energy_model=value_function_model).to(config.model.diffusion_model.device)

    value_function_model = torch.compile(value_function_model)
    energy_conditioned_diffusion_model = torch.compile(energy_conditioned_diffusion_model)
    ...
    if config.parameter.evaluation.model_save_path is not None:
        ...
        if not os.path.exists(config.parameter.evaluation.model_save_path):
            ...
            if len(checkpoint_files) == 0:
                ...
                energy_conditioned_diffusion_model.load_state_dict(checkpoint["diffusion_model"])
    ...
    data_loader = torch.utils.data.DataLoader(data, batch_size=config.parameter.unconditional_model.batch_size, shuffle=True)
    ...
    for train_iter in track(range(config.parameter.unconditional_model.iterations), description="unconditional_model training"):
        if train_iter < diffusion_model_iteration:
            continue

        train_data=next(data_generator).to(config.model.diffusion_model.device)
        train_x_0, train_x_1, train_value = train_data[:,:constraint_size], train_data[:,constraint_size:(constraint_size+x_size)], train_data[:,(constraint_size+x_size)]
        unconditional_model_training_loss = energy_conditioned_diffusion_model.score_matching_loss(x=train_x_1,condition=train_x_0)
        unconditional_model_optimizer.zero_grad()
        unconditional_model_training_loss.backward()
        unconditional_model_optimizer.step()
        moving_average_loss = 0.99 * moving_average_loss + 0.01 * unconditional_model_training_loss.item() if train_iter > 0 else unconditional_model_training_loss.item()
        if train_iter % 100 == 0:
            log.info(f"iteration {train_iter}, unconditional model loss {unconditional_model_training_loss.item()}, moving average loss {moving_average_loss}")
        ...
        if train_iter == 0 or (train_iter+1) % config.parameter.evaluation.eval_freq == 0:
            ...
            for guidance_scale in [0.0]:
                ...
                condition_x_0 = torch.tensor([1.0]).to(config.model.diffusion_model.device).unsqueeze(0)
                # condition_x_0 shape: [1, constraint_size]
                x_t = energy_conditioned_diffusion_model.sample_forward_process(t_span=t_span, batch_size=500, guidance_scale=guidance_scale, condition=condition_x_0).squeeze(2).cpu().detach()
                # x_t shape: [t_span, batch_size, x_size]
                condition_x_0 = condition_x_0.unsqueeze(0).expand(x_t.shape[0], 500, -1).cpu()
                # condition_x_0 shape: [t_span, batch_size, constraint_size]
                x_t = torch.concatenate([condition_x_0, x_t], dim=-1)
                # x_t shape: [t_span, batch_size, x_size+constraint_size]
    ...
    with Progress() as progress:
        value_training = progress.add_task("Value training", total=config.parameter.value_function_model.stop_training_iterations)
        ...
        for train_iter in range(config.parameter.energy_guidance.iterations):
            ...
            if train_iter < config.parameter.value_function_model.stop_training_iterations:
            ...
            train_fake_x_1 = energy_conditioned_diffusion_model.sample(
                t_span = torch.linspace(0.0, 1.0, 32).to(config.model.diffusion_model.device),
                batch_size=config.parameter.sample_per_data,
                condition=train_x_0,
                guidance_scale=0.0,
                with_grad=False,
            ).to(config.model.diffusion_model.device)

            train_fake_x_1 = torch.einsum("tbd->btd", train_fake_x_1)

            energy_guidance_loss = energy_conditioned_diffusion_model.energy_guidance_loss(x=train_fake_x_1, condition=train_x_0)
            energy_guidance_optimizer.zero_grad()
            energy_guidance_loss.backward()
            energy_guidance_optimizer.step()
            moving_average_energy_guidance_loss = 0.99 * moving_average_energy_guidance_loss + 0.01 * energy_guidance_loss.item() if train_iter > 0 else energy_guidance_loss.item()
            if train_iter % 100 == 0:
                log.info(f"iteration {train_iter}, energy guidance loss {energy_guidance_loss.item()}, moving average loss {moving_average_energy_guidance_loss}")
