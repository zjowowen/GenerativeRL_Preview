...
if __name__ == "__main__":
    ...
    reference_model = DiffusionModel(config=config.diffusion_model).to(
        config.diffusion_model.device
    )
    ...
    for i in range(pairs_num):
        # import pdb
        # pdb.set_trace()
        if abs(np.linalg.norm(data[i][:2], ord=2) - 3) <= abs(
            np.linalg.norm(another_data[i][:2], ord=2) - 3
        ):
    ...
    for iteration in track(range(config.parameter.iterations), description="Training"):
        ...
        if (
            iteration > 0
            and iteration % config.parameter.eval_freq == 0
            and iteration != 1000
        ):
        ...
        log.info(
            f"iteration {iteration}, gradient {gradient_sum/counter}, loss {loss.item()}"
        )
