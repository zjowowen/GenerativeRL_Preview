...
config = EasyDict(
    ...
    parameter=dict(
        ...
        diffusion_model_importance_sampling=dict(
            batch_size=2048,
            learning_rate=5e-5,
            clip_grad_norm=1.0,
            iterations=50000,
        ...
        evaluation=dict(
            eval_freq=5000,
            video_save_path="./video-swiss-roll-diffusion-model-important-sampling-with-value-model",
            model_save_path="./model-swiss-roll-diffusion-model-important-sampling-with-value-model",
        ),
    ),
)
...
def save_checkpoint(
    value_model,
    diffusion_model,
    diffusion_model_importance_sampling,
    diffusion_model_iteration,
    diffusion_model_importance_sampling_iteration,
    ...
    torch.save(
        dict(
            ...
            diffusion_model_importance_sampling=diffusion_model_importance_sampling.state_dict(),
            value_model_iteration=value_model_iteration,
            diffusion_model_iteration=diffusion_model_iteration,
            diffusion_model_importance_sampling_iteration=diffusion_model_importance_sampling_iteration,
        ),
        f=os.path.join(
            path,
            f"checkpoint_{value_model_iteration+diffusion_model_iteration+diffusion_model_importance_sampling_iteration}.pt",
        ),
    )
...
if __name__ == "__main__":
    ...
    diffusion_model_importance_sampling = DiffusionModel(
    ...
    diffusion_model_importance_sampling = torch.compile(
        diffusion_model_importance_sampling
    )
    ...
    if config.parameter.evaluation.model_save_path is not None:
        ...
        if not os.path.exists(config.parameter.evaluation.model_save_path):
            ...
            diffusion_model_importance_sampling_iteration = 0
        else:
            checkpoint_files = [
                f
            ...
            if len(checkpoint_files) == 0:
                ...
                diffusion_model_importance_sampling.load_state_dict(
                    checkpoint["diffusion_model_importance_sampling"]
                )
                ...
                diffusion_model_importance_sampling_iteration = checkpoint.get(
                    "diffusion_model_importance_sampling_iteration", 0
                )
    else:
        diffusion_model_iteration = 0
    ...
    data_loader = torch.utils.data.DataLoader(
        data,
        batch_size=config.parameter.value_function_model.batch_size,
        shuffle=True,
        drop_last=True,
    )
    ...
    for train_iter in track(
        range(config.parameter.value_function_model.iterations),
        ...
        if (
            ...
            save_checkpoint(
                ...
                diffusion_model_importance_sampling=diffusion_model_importance_sampling,
                value_model_iteration=value_model_iteration,
                diffusion_model_iteration=diffusion_model_iteration,
                diffusion_model_importance_sampling_iteration=diffusion_model_importance_sampling_iteration,
    ...
    data_loader = torch.utils.data.DataLoader(
        data,
        batch_size=config.parameter.diffusion_model.batch_size,
        shuffle=True,
        drop_last=True,
    )
    data_generator = get_train_data(data_loader)
        ...
        if (train_iter + 1) % config.parameter.evaluation.eval_freq == 0:
            ...
            p = mp.Process(
                ...
                args=(
                    x_t,
                    config.parameter.evaluation.video_save_path,
                    f"value_model_iteration_{value_model_iteration}_diffusion_model_iteration_{diffusion_model_iteration}_diffusion_model_importance_sampling_iteration_{diffusion_model_importance_sampling_iteration}",
                    100,
                    100,
                ),
            )
    ...
    fake_x = generate_fake_x(diffusion_model, config.parameter.sample_per_data)

    data_fake_x = fake_x.detach().cpu().numpy()
    data_loader_fake_x = torch.utils.data.DataLoader(
        data_fake_x,
        batch_size=config.parameter.diffusion_model_importance_sampling.batch_size,
        shuffle=True,
        drop_last=True,
    )
    ...
    diffusion_model_importance_sampling_optimizer = torch.optim.Adam(
        diffusion_model_importance_sampling.model.parameters(),
        lr=config.parameter.diffusion_model_importance_sampling.learning_rate,
    )
    ...
    for train_iter in track(
        range(config.parameter.diffusion_model_importance_sampling.iterations),
        description="diffusion model training",
    ):
        if train_iter < diffusion_model_importance_sampling_iteration:
            continue

        train_data = next(data_generator).to(config.model.diffusion_model.device)
        train_x, train_value = train_data[:, :x_size], train_data[:, x_size]
        diffusion_model_importance_sampling_training_loss = (
            diffusion_model_importance_sampling.score_matching_loss(
                train_x, average=False
            )
        )
        fake_x = next(data_generator_fake_x).to(config.model.diffusion_model.device)
        fake_value = value_function_model(fake_x).squeeze(dim=-1)
        fake_value_softmax = torch.sum(
            softmax(1 * fake_value) * fake_value,
            dim=-1,
            keepdim=True,
        ).squeeze(dim=-1)
        # value = value_function_model(train_x).squeeze(dim=-1)

        diffusion_model_importance_sampling_training_loss = torch.mean(
            diffusion_model_importance_sampling_training_loss
            * torch.exp(train_value - fake_value_softmax)
        )

        diffusion_model_importance_sampling_optimizer.zero_grad()
        diffusion_model_importance_sampling_training_loss.backward()
        gradien_norm = torch.nn.utils.clip_grad_norm_(
            diffusion_model_importance_sampling.parameters(),
            config.parameter.diffusion_model_importance_sampling.clip_grad_norm,
        )
        diffusion_model_importance_sampling_optimizer.step()
        moving_average_loss = (
            0.99 * moving_average_loss
            + 0.01 * diffusion_model_importance_sampling_training_loss.item()
            if train_iter > 0
            else diffusion_model_importance_sampling_training_loss.item()
        )

        if train_iter % 100 == 0:
            log.info(
                f"iteration {train_iter}, gradient norm {gradien_norm}, unconditional model loss {diffusion_model_importance_sampling_training_loss.item()}, moving average loss {moving_average_loss}"
            )

        diffusion_model_importance_sampling_iteration = train_iter

        if (train_iter + 1) % config.parameter.evaluation.eval_freq == 0:
            diffusion_model_importance_sampling.eval()

            t_span = torch.linspace(0.0, 1.0, 1000)
            x_t = (
                diffusion_model_importance_sampling.sample_forward_process(
