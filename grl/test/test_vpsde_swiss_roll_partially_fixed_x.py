...
if __name__ == "__main__":
    ...
    def render_video(data_list, video_save_path, iteration, fps=100, dpi=100, info=None):
        ...
        if info is not None:
            ani.save(os.path.join(video_save_path, f"iteration_{iteration}_{info}.mp4"), fps=fps, dpi=dpi)
        else:
    ...
    for iteration in track(range(config.parameter.iterations), description="Training"):
        ...
        if iteration > 0 and iteration % config.parameter.eval_freq == 0:
            ...
            x_t=[x.squeeze(0) for x in torch.split(x_t, split_size_or_sections=1, dim=0)]
            # render_video(x_t, config.parameter.video_save_path, iteration, fps=100, dpi=100)
            p2 = mp.Process(target=render_video, args=(x_t, config.parameter.video_save_path, iteration, 100, 100))
            p2.start()
