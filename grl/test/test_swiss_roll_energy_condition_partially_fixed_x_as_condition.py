        ...
        diffusion_model=dict(
            ...
            energy_guidance=dict(
                ...
                backbone=dict(
                    ...
                    args=dict(
                        hidden_sizes=[
                            x_size + constraint_size + t_embedding_dim,
                            256,
                            256,
                        ],
                        output_size=1,
...
if __name__ == "__main__":
    ...
    for train_iter in track(
        ...
        train_x_0, train_x_1, train_value = (
            train_data[:, :constraint_size],
            train_data[:, constraint_size : (constraint_size + x_size)],
            train_data[:, (constraint_size + x_size)],
        )
        unconditional_model_training_loss = (
            energy_conditioned_diffusion_model.score_matching_loss(
                x=train_x_1, condition=train_x_0
            )
        )
        ...
        if (
            ...
            for guidance_scale in [0.0]:
                ...
                condition_x_0 = (
                    condition_x_0.unsqueeze(0).expand(x_t.shape[0], 500, -1).cpu()
                )
        ...
        for train_iter in range(config.parameter.energy_guidance.iterations):
            ...
            energy_guidance_loss = (
                energy_conditioned_diffusion_model.energy_guidance_loss(
                    x=train_fake_x_1, condition=train_x_0
                )
            )
