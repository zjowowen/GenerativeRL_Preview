...
class MinariDataset(MinariDataset):
    ...
    def __init__(
        ...
        # merge the episodes
        tmp_dataset = {'observations': np.zeros((0, dataset[0].observations.shape[1])), \
            'actions': np.zeros((0, dataset[0].actions.shape[1])),\
            'rewards': np.zeros(0),\
            'terminals': np.zeros(0)}
        for episode in range(dataset.__len__()):
            
            tmp_dataset['observations'] = np.vstack((tmp_dataset['observations'], dataset[episode].observations[:-1]))
            tmp_dataset['actions'] = np.vstack((tmp_dataset['actions'], dataset[episode].actions))
            tmp_dataset['rewards'] = np.hstack((tmp_dataset['rewards'], dataset[episode].rewards))
            tmp_dataset['terminals'] = np.hstack((tmp_dataset['terminals'], dataset[episode].terminations))
