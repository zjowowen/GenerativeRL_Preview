...
class Policy(nn.Module):
    ...
    def forward(self, state: Union[torch.Tensor, TensorDict],tanh=False) -> Union[torch.Tensor, TensorDict]:
        """
        Overview:
        ...
        if tanh==True:
            ...
            logp = logp_1 - (2*(np.log(2) - action - F.softplus(-2*action))).sum(axis=1)
            ...
            **config.wandb if hasattr(config, "wandb") else {}
            ...
            for online_rl_iteration in track(range(config.parameter.online_rl.iterations), description="Online RL iteration"):
                ...
                def process_batch(replay_buffer, config):
                    ...
                    batch = replay_buffer.sample(batch_size=config.parameter.online_rl.batch_size).to(config.model.Policy.device)
                ...
                for i in range(10):
                        state,action,reward,next_state,done = process_batch(self.replay_buffer,config)
                        ...
                        wandb.log({"logp_mean":logp.mean(),"logp_min":logp.min(),"logp_max":logp.max()},commit=False)
