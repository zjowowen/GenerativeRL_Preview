...
class Policy(nn.Module):
    ...
    def __init__(self, config: EasyDict):
        ...
        self.model_loss_type = config.policy.model_loss_type
        assert self.model_loss_type in ["score_matching", "flow_matching"]

    def forward(
        self, state: Union[torch.Tensor, TensorDict], tanh=False
        ...
        action = self.policy.sample(t_span=t_span, condition=state, with_grad=True)
        ...
        if tanh == True:
            ...
            logp_1 = logp.sum(axis=-1)
            logp = logp_1 - (2 * (np.log(2) - action - F.softplus(-2 * action))).sum(
                axis=1
            )
    ...
    def sample(
        ...
        """
        t_span = torch.linspace(0, 1, 5)
        action = self.policy.sample(t_span=t_span, condition=state, with_grad=False)
        return action
    ...
    def behaviour_policy_loss(
        ...
        if self.model_loss_type == "score_matching":
            if maximum_likelihood:
                return self.policy.score_matching_loss(action, state)
            else:
                return self.policy.score_matching_loss(
                    action, state, weighting_scheme="vanilla"
                )
        elif self.model_loss_type == "flow_matching":
            return self.policy.flow_matching_loss(action, state)
    ...
    def train(self, config: EasyDict = None):
        ...
        with wandb.init(
            ...
            **config.wandb if hasattr(config, "wandb") else {},
            ...
            flow_matching_optimizer = torch.optim.Adam(
            ...
            def collate_to_tensordict(data_list):
                ...
                for item in data_list:
                    ...
                    next_obs_list.append(
                        torch.tensor(item["next_obs"], dtype=torch.float32)
                    )
            ...
            for online_rl_iteration in track(
                ...
                if (
                    ...
                    log.info(
                        f"online_rl_iteration: {online_rl_iteration}:"
                        + "".join(
                            [
                                f"{key}: {value}, "
                ...
                def process_batch(replay_buffer, config):
                    # Sample a batch from the replay buffer
                    batch = replay_buffer.sample(
                        batch_size=config.parameter.online_rl.batch_size
                    ).to(config.model.Policy.device)
                    # Extract the state, action, reward, next state, and done flag from the batch
                    state = batch["obs"]
                    action = (
                ...
                for i in range(10):
                    state, action, reward, next_state, done = process_batch(
                        self.replay_buffer, config
                    )
                    flow_matching_optimizer.zero_grad()
                    behaviour_policy_loss = self.model["Policy"].behaviour_policy_loss(
                        action,
                        state,
                        maximum_likelihood=False,
                    )
                    behaviour_policy_loss.backward()
                    action, logp = self.model["Policy"].forward(state)
                    wandb.log(
                        {
                            "logp_mean": logp.mean(),
                            "logp_min": logp.min(),
                ...
                for _ in range(config.parameter.online_rl.update_steps):
                    ...
                    entropy_coeffi_loss, average_action_entropy = (
                        compute_entropy_coeffi_loss(state, self.model["Policy"])
                    )
