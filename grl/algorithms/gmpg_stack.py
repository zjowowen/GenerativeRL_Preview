...
class GMPGAlgorithm:
    ...
    def train(self, config: EasyDict = None, seed=None):
        ...
        with wandb.init(**config.wandb) as wandb_run:
            ...
            #     batch_size=config.parameter.behaviour_policy.batch_size*config.parameter.number_stack_time_step,
            #     sampler=SliceSamplerWithoutReplacement(traj_key="episode", num_slices=config.parameter.behaviour_policy.batch_size),
            #     prefetch=10,
            #     pin_memory=True,
            # )
            ...
            for epoch in track(
                ...
                for index, data in enumerate(replay_buffer):
                    ...
                    data = data.to(config.model.GPPolicy.device)

                    # action = data["a"].reshape(-1, config.parameter.number_stack_time_step, *data["a"].shape[1:])[:,-1]
                    # state = data["s"].reshape(-1, config.parameter.number_stack_time_step, *data["s"].shape[1:])
                    # next_state = data["s_"].reshape(-1, config.parameter.number_stack_time_step, *data["s_"].shape[1:])
                    # reward = data["r"].reshape(-1, config.parameter.number_stack_time_step, *data["r"].shape[1:])[:,-1].unsqueeze(-1).float()
                    # done = data["d"].reshape(-1, config.parameter.number_stack_time_step, *data["d"].shape[1:])[:,-1].unsqueeze(-1)
            ...
            # replay_buffer=TensorDictReplayBuffer(
            #     storage=self.dataset.storage,
            #     batch_size=config.parameter.critic.batch_size*config.parameter.number_stack_time_step,
            #     sampler=SliceSamplerWithoutReplacement(traj_key="episode", num_slices=config.parameter.critic.batch_size),
            #     prefetch=10,
            #     pin_memory=True,
            # )
            ...
            for epoch in track(
                ...
                for index, data in enumerate(replay_buffer):
                    ...
                    state = data["s"]
                    next_state = data["s_"]
                    reward = data["r"].unsqueeze(-1).float()
                    done = data["d"].unsqueeze(-1)
            ...
            # replay_buffer=TensorDictReplayBuffer(
            #     storage=self.dataset.storage,
            #     batch_size=config.parameter.guided_policy.batch_size*config.parameter.number_stack_time_step,
            #     # sampler=SliceSamplerWithoutReplacement(traj_key="episode", num_slices=config.parameter.guided_policy.batch_size),
            #     sampler=SliceSamplerWithoutReplacement(traj_key="episode", slice_len=config.parameter.number_stack_time_step),
            #     prefetch=10,
            #     pin_memory=True
            # )
            ...
            for epoch in track(
                ...
                for index, data in enumerate(replay_buffer):
                    ...
                    guided_policy_loss = guided_policy_loss * (
                        state.shape[0] / config.parameter.guided_policy.batch_size
                    )
