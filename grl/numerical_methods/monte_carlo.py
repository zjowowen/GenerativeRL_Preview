...
class MonteCarloSampler:
    ...
    def plot_samples(self, samples, num_bins=50):
        ...
        plt.hist(samples.detach().cpu().numpy(), bins=num_bins, density=True, alpha=0.5, label='Monte Carlo samples')
        x = torch.linspace(self.x_min, self.x_max, 1000)
        normalized_pdf = self.unnormalized_pdf(x) / torch.trapz(self.unnormalized_pdf(x), x)
