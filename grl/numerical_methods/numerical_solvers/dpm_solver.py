...
class DPMSolver:
    ...
    def __init__(
        ...
        self.method = method
        assert self.method in ['singlestep', 'multistep', 'singlestep_fixed', 'adaptive']
    ...
    def integrate(
        ...
        def model_fn(x: Union[torch.Tensor, TensorDict], t: torch.Tensor) -> Union[torch.Tensor, TensorDict]:
            """
            Overview:
        ...
        def get_time_steps(t_T, t_0, N):
            ...
            if self.skip_type == 'logSNR':
                ...
                t_order = 2
                t = torch.linspace(t_T ** (1. / t_order), t_0 ** (1. / t_order), N + 1).pow(t_order).to(self.device)
                return t
            else:
                raise ValueError(
                    "Unsupported skip_type {}, need to be 'logSNR' or 'time_uniform' or 'time_quadratic'".format(self.skip_type)
                )
        ...
        def denoise_fn(x: Union[torch.Tensor, TensorDict], s: torch.Tensor, condition: Union[torch.Tensor, TensorDict]) -> Union[torch.Tensor, TensorDict]:
            """
            Overview:
        ...
        def dpm_solver_first_update(x: Union[torch.Tensor, TensorDict], s: torch.Tensor, t: torch.Tensor, model_s: Union[torch.Tensor, TensorDict]=None, return_intermediate: bool=False):
            """
            Overview:
        ...
        def singlestep_dpm_solver_second_update(
            x: Union[torch.Tensor, TensorDict], s: torch.Tensor, t: torch.Tensor, r1: float=0.5, model_s: Union[torch.Tensor, TensorDict]=None, return_intermediate: bool=False
        ):
            """
            Overview:
            ...
            if self.use_dpm_solver_plus_plus:
                ...
                if self.default_high_order_solvers:
                    ...
                    x_t = sigma_t / sigma_s * x - alpha_t * phi_1 * model_s + (1. / r1) * alpha_t * ((torch.exp(-h) - 1.) / h + 1.) * (model_s1 - model_s)
            else:
                phi_11 = torch.expm1(r1 * h)
                phi_1 = torch.expm1(h)
                ...
                if self.default_high_order_solvers:
                    ...
                    x_t = torch.exp(log_alpha_t - log_alpha_s) * x - sigma_t * phi_1 * model_s - (1. / r1) * sigma_t * ((torch.exp(h) - 1.) / h - 1.) * (model_s1 - model_s)
            if return_intermediate:
        ...
        def singlestep_dpm_solver_third_update(
            ...
            if self.use_dpm_solver_plus_plus:
                ...
                if self.default_high_order_solvers:
                    x_t = sigma_t / sigma_s * x - alpha_t * phi_1 * model_s + (1. / r2) * alpha_t * phi_2 * (model_s2 - model_s)
                else:
                ...
                model_s2 = model_fn(x_s2, s2)
                if self.default_high_order_solvers:
                    x_t = torch.exp(log_alpha_t - log_alpha_s) * x - sigma_t * phi_1 * model_s - (1. / r2) * sigma_t * phi_2 * (model_s2 - model_s)
                else:
                    D1_0 = (1. / r1) * (model_s1 - model_s)
                    D1_1 = (1. / r2) * (model_s2 - model_s)
                    D1 = (r2 * D1_0 - r1 * D1_1) / (r2 - r1)
        ...
        def multistep_dpm_solver_second_update(x: Union[torch.Tensor, TensorDict], model_prev_list: List[Union[torch.Tensor, TensorDict]], t_prev_list: List[torch.Tensor], t: torch.Tensor):
            """
            Overview:
            ...
            if self.use_dpm_solver_plus_plus:
                if self.solver_type == 'dpm_solver':
                    x_t = sigma_t / sigma_prev_0 * x - alpha_t * (torch.exp(-h) - 1.) * model_prev_0 - 0.5 * alpha_t * (torch.exp(-h) - 1.) * D1_0
                elif self.solver_type == 'taylor':
                    x_t = sigma_t / sigma_prev_0 * x - alpha_t * (torch.exp(-h) - 1.) * model_prev_0 + alpha_t * ((torch.exp(-h) - 1.) / h + 1.) * D1_0
            else:
                if self.solver_type == 'dpm_solver':
                    x_t =  torch.exp(log_alpha_t - log_alpha_prev_0) * x - sigma_t * (torch.exp(h) - 1.) * model_prev_0 - 0.5 * sigma_t * (torch.exp(h) - 1.) * D1_0
                elif self.solver_type == 'taylor':
                    x_t = torch.exp(log_alpha_t - log_alpha_prev_0) * x - sigma_t * (torch.exp(h) - 1.) * model_prev_0 - sigma_t * ((torch.exp(h) - 1.) / h - 1.) * D1_0
            return x_t

        def multistep_dpm_solver_third_update(x: Union[torch.Tensor, TensorDict], model_prev_list: List[Union[torch.Tensor, TensorDict]], t_prev_list: List[torch.Tensor], t: torch.Tensor):
            """
            Overview:
            ...
            if self.use_dpm_solver_plus_plus:
                x_t = sigma_t / sigma_prev_0 * x - alpha_t * (torch.exp(-h) - 1.) * model_prev_0 + alpha_t * ((torch.exp(-h) - 1.) / h + 1.) * D1 - alpha_t * ((torch.exp(-h) - 1. + h) / h ** 2 - 0.5) * D2
            else:
                x_t = torch.exp(log_alpha_t - log_alpha_prev_0) * x - sigma_t * (torch.exp(h) - 1.) * model_prev_0 - sigma_t * ((torch.exp(h) - 1.) / h - 1.) * D1 - sigma_t * ((torch.exp(h) - 1. - h) / h ** 2 - 0.5) * D2
            return x_t

        def singlestep_dpm_solver_update(
            x: Union[torch.Tensor, TensorDict], s: torch.Tensor, t: torch.Tensor, order: float, return_intermediate: bool=False, r1: float=None, r2: float=None
        ):
            """
            Overview:
            ...
            if order == 1:
                ...
                return singlestep_dpm_solver_second_update(x, s, t, return_intermediate=return_intermediate, r1=r1)
            elif order == 3:
                return singlestep_dpm_solver_third_update(x, s, t, return_intermediate=return_intermediate, r1=r1, r2=r2)
            else:
                raise ValueError("Solver order must be 1 or 2 or 3, got {}".format(order))

        def multistep_dpm_solver_update(x: Union[torch.Tensor, TensorDict], model_prev_list: List[Union[torch.Tensor, TensorDict]], t_prev_list: List[torch.Tensor], t: torch.Tensor, order: int):
            """
            Overview:
            ...
            if order == 1:
                return dpm_solver_first_update(x, t_prev_list[-1], t, model_s=model_prev_list[-1])
            elif order == 2:
                return multistep_dpm_solver_second_update(x, model_prev_list, t_prev_list, t)
            elif order == 3:
                return multistep_dpm_solver_third_update(x, model_prev_list, t_prev_list, t)
            else:
                raise ValueError("Solver order must be 1 or 2 or 3, got {}".format(order))

        def dpm_solver_adaptive(
            x: Union[torch.Tensor, TensorDict], t_T: float, t_0: float, h_init: float=0.05, theta: float=0.9, t_err: float=1e-5, save_intermediate: bool=False
        ):
            """
            Overview:
            ...
            if self.order == 2:
                r1 = 0.5
                lower_update = lambda x, s, t: dpm_solver_first_update(x, s, t, return_intermediate=True)
                higher_update = lambda x, s, t, **kwargs: singlestep_dpm_solver_second_update(x, s, t, r1=r1, **kwargs)
            elif self.order == 3:
                r1, r2 = 1. / 3., 2. / 3.
                lower_update = lambda x, s, t: singlestep_dpm_solver_second_update(x, s, t, r1=r1, return_intermediate=True)
                higher_update = lambda x, s, t, **kwargs: singlestep_dpm_solver_third_update(x, s, t, r1=r1, r2=r2, **kwargs)
            else:
                raise ValueError("For adaptive step size solver, order must be 2 or 3, got {}".format(self.order))
            while torch.abs((s - t_0)).mean() > t_err:
                t = diffusion_process.InverseHalfLogSNR(HalfLogSNR=lambda_s + h)[:, 0]
                x_lower, lower_noise_kwargs = lower_update(x, s, t)
                x_higher = higher_update(x, s, t, **lower_noise_kwargs)
                delta = torch.max(torch.ones_like(x).to(x) * self.atol, self.rtol * torch.max(torch.abs(x_lower), torch.abs(x_prev)))
                ...
                h = torch.min(theta * h * torch.float_power(E, -1. / self.order).float(), lambda_0 - lambda_s)
                self.nfe += self.order
            if save_intermediate:
                return x, x_list
            else:
        ...
        if self.method == 'adaptive':
            ...
            with torch.no_grad():
                ...
                for init_order in range(1, self.order):
                    ...
                    x = multistep_dpm_solver_update(x, model_prev_list, t_prev_list, vec_t, init_order)
                    if save_intermediate:
                ...
                for step in range(self.order, steps + 1):
                    ...
                    x = multistep_dpm_solver_update(x, model_prev_list, t_prev_list, vec_t, self.order)
                    if save_intermediate:
                        x_list.append(x.clone())
            ...
            with torch.no_grad():
                ...
                for order in orders:
                    ...
                    h = diffusion_process.HalfLogSNR(t=timesteps[i + order]) - diffusion_process.HalfLogSNR(t=timesteps[i])
                    ...
                    x = singlestep_dpm_solver_update(x, vec_s, vec_t, order, r1=r1, r2=r2)
                    if save_intermediate:
                        x_list.append(x.clone())
                    i += order
        if self.denoise:
