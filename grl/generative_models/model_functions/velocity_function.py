    ...
    def flow_matching_loss(
        ...
        def get_loss(velocity_value, velocity):
            if isinstance(velocity_value, torch.Tensor):
                if average:
                    return torch.mean(torch.sum(0.5 * (velocity_value - velocity) ** 2, dim=(1, )))
                else:
                ...
                if average:
                    return treetensor.torch.mean(treetensor.torch.sum(0.5 * (velocity_value - velocity) * (velocity_value - velocity), dim=(1, )))
                else:
                    return treetensor.torch.sum(0.5 * (velocity_value - velocity) * (velocity_value - velocity), dim=(1, ))
            else:
