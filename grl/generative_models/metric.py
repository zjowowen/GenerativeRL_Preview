...
def compute_likelihood_2(
    ...
    def composite_drift(t, x):
        ...
        with torch.set_grad_enabled(True):
            ...
            delta_x = treetensor.torch.tensor({}, device=x_t.device)
            delta_x["x"] = dx
            delta_x["logp_xt_minus_logp_x0"] = logp_drift
            return delta_x

    # x.shape = [batch_size, state_dim]
    x0_and_diff_logp = treetensor.torch.tensor(
        {"x": x, "logp_xt_minus_logp_x0": torch.zeros(x.shape[0])}, device=x.device
    )
