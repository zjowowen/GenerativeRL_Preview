...
class SchrodingerBridgeConditionalFlowModel(nn.Module):
    ...
    def __init__(
        ...
        self.velocity_model = IntrinsicModel(config.velocity_model.args)
        self.score_model = IntrinsicModel(config.score_model.args)
    ...
    def get_type(self):
        return "SchrodingerBridgeConditionalFlowModel"
    ...
    def sample_forward_process(
        ...
        if isinstance(solver, DPMSolver):
            ...
            def drift(t, x):
                return self.velocity_model(t=t, x=x, condition=condition)
            ...
            if solver.library == "torchdiffeq_adjoint":
                if with_grad:
                    data = solver.integrate(
                        ...
                        x0=x,
                        t_span=t_span,
                        adjoint_params=find_parameters(self.velocity_model),
                    )
            else:
                with torch.no_grad():
                    data = solver.integrate(
    ...
    def flow_matching_loss(
        ...
        vetority_loss, score_loss = self.function_.SchrodingerBridge_loss(
            self.velocity_model,
            self.score_model,
            x0_ot,
            x1_ot,
...
class SchrodingerBridgeGuidedConditionalFlowModel:
    ...
    def __init__(
        ...
        if hasattr(config, "reverse_path"):
            ...
            self.reverse_path = None
        self.model_type = config.velocity_model.type
